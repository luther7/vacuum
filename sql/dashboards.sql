WITH candles AS (
  SELECT
    ...
    max(price) OVER (PARTITION BY floor(event_time/1000/60)::int * 60 ORDER BY time) AS high,
    min(price) OVER (PARTITION BY floor(event_time/1000/60)::int * 60 ORDER BY time) AS low,
    first_value(price) OVER (PARTITION BY floor(event_time/1000/60)::int * 60 ORDER BY time) AS open,
    last_value(price) OVER (PARTITION BY floor(event_time/1000/60)::int * 60 ORDER BY time) AS close
  FROM
    binance_trade
  WHERE
...
WITH averages AS (
  SELECT
    ...
    avg(price) OVER (PARTITION BY floor(event_time/1000/60)::int * 60 ORDER BY time) AS price,
  FROM
    bitforex_trade
  WHERE
    event_time/1000 >= $__unixEpochFrom()
